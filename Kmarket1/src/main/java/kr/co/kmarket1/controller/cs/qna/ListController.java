...
public class ListController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 전체 게시물 갯수
		if(cate == null) {
			total = CsDAO.getInstance().selectCountQnas();
		}else {
			total = CsDAO.getInstance().selectCountQna(cate);
		}
		...
		if(cate == null) {
			articles = CsDAO.getInstance().selectQnasGroup(start);
		}else {
			articles = CsDAO.getInstance().selectQnasCate(cate, start);
		}
